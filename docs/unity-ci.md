...
3. **Вторая нога — другой образ эмулятора.** Job `smoke-retry` запускается
   ...
   API 33 (`api-level: 33` в матрице job'а; `ANDROID_BACKEND` печатается как
   `github-hosted-emulator-api33-retry`). Это повтор *бэкенда*, а не
   ослабление проверки: скрипт тот же, ассершен тот же. Ранее здесь был
   указан API 34 — документация разошлась с workflow.
...
## Прогон 30 (2026-09-10, head `ec3b7cd`) и дыра в focus gate

Прогон `unity-android.yml` № 30 (run id 34503965758) — пятый подряд фейл
device-лега и первый, где причина видна полностью, а не угадывается.

Сборка, установка и запуск зелёные: APK 42 041 135 байт, `Performing Streamed
Install / Success`, `am start Status: ok`, `LaunchState: COLD`,
`TotalTime 12352 ms`. Дальше `tools/android-smoke.sh` напечатал
`INFRA_FAIL: the app process died before it finished loading` и вышел с кодом 1:

```text
Unity   : onDestroy
Unity   : Quitting process
Zygote  : Process 2244 exited due to signal 9 (Killed)
ActivityManager: Process com.jotalbot.words (pid 2244) has died: fg  TOP
```

Это INFRA_FAIL, а не PRODUCT_FAIL: в том же минутном окне эмулятор убивал и
посторонние системные процессы — `init: Sending signal 9 to service 'idmap2d'`,
`Bluetooth remote service has died`, `Process com.android.phone has died`.
Одновременный SIGKILL нескольких несвязанных процессов — это нехватка памяти на
хосте, а не дефект обработки жеста. До ассершена `WORDS_SWIPE` прогон не дошёл,
поэтому реализация жеста этим прогоном neither подтверждена, ни опровергнута.

Нога `smoke-retry` (API 33) была **cancelled**, а не failed: job `smoke-gate`
снимает вторую ногу, как только первая дала вердикт. Значит прогон дал одну
измеримую точку, а не две.

### Найденный дефект самого harness'а

Тот же лог показал, что focus gate пропускал случай, для которого был написан.
Gate принимал `mCurrentFocus=Window{... $PKG` **или** `mFocusedApp.*$PKG`, и в
прогоне 30 он прошёл по второй альтернативе, тогда как следующая же
диагностическая строка напечатала

```text
mCurrentFocus=Window{7a31fc3 u0 ImmersiveModeConfirmation}
```

— системный лист первого запуска стоял поверх доски. `mFocusedApp` остаётся на
нашем пакете даже тогда, когда окном, реально принимающим ввод, владеет система,
поэтому эта альтернатива сообщала об успехе ровно в том случае, который gate
должен был отловить.

Исправление (batch 23B): gate читает только `mCurrentFocus` и сопоставляет
` $PKG/`, то есть именно наше activity, а не любое окно, упоминающее пакет.
Поведение проверено на точных строках из прогона 30:

| ситуация | старый gate | новый gate |
|---|---|---|
| системный лист поверх приложения (прогон 30) | PASS (ложно) | FAIL → INFRA_FAIL |
| приложение реально держит фокус | PASS | PASS |
| информации об окнах нет вообще | PASS (ложно) | FAIL |

Побочный эффект честный: device-лег может краснеть чаще, но теперь он краснеет
по правильной причине и с правильной диагностикой.
